...
function readPolicyFromDisk() {
  ...
  if (forceCI && !experimentalPolicy) {
    throw new ERR_MANIFEST_ASSERT_INTEGRITY("UMCI is forced by system policy. A Policy manifest is required --experimental-policy", "Invalid manifest");
  }
  ...
  if (experimentalPolicy) {
    ...
    if (forceCI)
    {
      let manifestSignatureURL = new URL(manifestURL.href + ".p7");
      let isTrusted = os.isFileTrustedBySystemCodeIntegrity(fileURLToPath(manifestURL), fileURLToPath(manifestSignatureURL))
      if (!isTrusted) {
        throw new ERR_MANIFEST_ASSERT_INTEGRITY(returnCode, "Invalid manifest");
      }
    } else if (experimentalPolicyIntegrity) {
