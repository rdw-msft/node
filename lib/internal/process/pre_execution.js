...
function readPolicyFromDisk() {
  ...
  let forceCI = os.isCIEnforcedByOS();
  if (forceCI && !experimentalPolicy) {
    throw new ERR_MANIFEST_SYSTEM_CI_VIOLATION("Code integrity is forced by system policy. A Policy manifest (--experimental-policy) and signature (--policy-signature) are required");
  }
  ...
  if (experimentalPolicy || forceCI) {
    ...
    let manifestURL = makeAbsoluteUrl(experimentalPolicy)
    ...
    if (policySignature) {
      let policySignatureUrl = makeAbsoluteUrl(policySignature);

      let isPolicyTrusted = os.isFileTrustedBySystemCodeIntegrity(fileURLToPath(manifestURL), fileURLToPath(policySignatureUrl))
      if (!isPolicyTrusted) {
        throw new ERR_MANIFEST_SYSTEM_CI_VIOLATION("The --policy-signature provided is not valid or does not meet system code integrity requirements");
      }
    }
